    ...
    def _validate(
        ...
        if isfile(path):
            ...
            with smart_open(path, "r") as query_file:
                ...
                # Escape single quotes in the path so it can be safely
                # embedded as a SQL string literal.  This avoids relying on
                # Spark's named-parameter binding (`:name` / `args=`) which
                # behaves inconsistently across Spark 3.x and 4.x engines
                # and in Hive-enabled sessions.
