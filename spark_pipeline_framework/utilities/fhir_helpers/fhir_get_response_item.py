...
from spark_pipeline_framework.utilities.fhir_helpers.fhir_get_response_schema import FhirGetResponseSchema


@dataclasses.dataclass
class FhirGetResponseItem:
    def __init__(self, item: Dict[str, Any]) -> None:
        self.partition_index: int = item.get(FhirGetResponseSchema.partition_index)
        self.sent: int = item.get(FhirGetResponseSchema.sent)
        self.received: int = item.get(FhirGetResponseSchema.received)
        ...
        self.extra_context_to_return: Optional[str] = item.get(FhirGetResponseSchema.extra_context_to_return)
