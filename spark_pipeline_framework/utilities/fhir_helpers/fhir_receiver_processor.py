...
from spark_pipeline_framework.transformers.fhir_receiver.v2.fhir_receiver_parameters import FhirReceiverParameters
from spark_pipeline_framework.utilities.fhir_helpers.fhir_get_response_item import FhirGetResponseItem
...
class FhirReceiverProcessor:
    ...
    def send_partition_request_to_server(
        ...
        resource_id_with_token_list: List[Dict[str, Optional[str]]] = [
            (
                ...
                if parameters.has_token_col and parameters.url_column and parameters.slug_column
                else (
                    {
                        "resource_id": r["id"],
                        "access_token": r["token"],
                    ...
                    else {"resource_id": r["id"], "access_token": parameters.auth_access_token}
                )
            )
            for r in rows
        ]
        result = FhirReceiverHelpers.process_with_token(
            ...
            has_token_col=parameters.has_token_col,
            server_url=parameters.server_url,
            log_level=parameters.log_level,
            action=parameters.action,
            action_payload=parameters.action_payload,
            additional_parameters=parameters.additional_parameters,
            filter_by_resource=parameters.filter_by_resource,
            filter_parameter=parameters.filter_parameter,
            ...
            expand_fhir_bundle=parameters.expand_fhir_bundle,
            accept_type=parameters.accept_type,
            content_type=parameters.content_type,
            additional_request_headers=parameters.additional_request_headers,
            accept_encoding=parameters.accept_encoding,
            slug_column=parameters.slug_column,
            ...
            error_view=parameters.error_view,
            url_column=parameters.url_column,
            use_data_streaming=parameters.use_data_streaming,
            graph_json=parameters.graph_json,
            refresh_token_function=parameters.refresh_token_function,
        )
        return result
    ...
    def process_with_token(
        ...
        if parameters.batch_size and parameters.batch_size > 1 and not parameters.has_token_col:
    ...
    def process_batch(
        ...
        responses_from_fhir = []
        try:
            responses_from_fhir = FhirReceiverProcessor.json_str_to_list_str(resp_result)
